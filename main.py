...
try:
    if os.path.exists(detection_model_path):
        ...
        print(f"Warning: Detection model not found at {detection_model_path}. Please ensure it exists.")
    ...
    if os.path.exists(classification_model_path):
        ...
        print(f"Warning: Classification model not found at {classification_model_path}. Please ensure it exists.")

    if detection_model and classification_model:
        print("All models loaded successfully!")
    else:
        print("Some models could not be loaded. Check paths and file existence.")
except Exception as e:
    print(f"Error loading models: {e}")

last_detect_label = None

# --- Helper function to read image from bytes ---
...
async def detect_device(file: UploadFile = File(...)):
    global last_detect_label

    if detection_model is None:
        raise HTTPException(status_code=500, detail="Detection model not loaded. Check server logs.")
    ...
    if not results or not results[0].boxes or len(results[0].boxes) == 0:
        last_detect_label = "no_device_detected"
        return {
            "label": last_detect_label,
            "bbox": [],
    ...
    if 0 <= int(cls_id) < len(detection_model.names):
        label = detection_model.names[int(cls_id)]
    else:
        label = "unknown_device"
    last_detect_label = label
    ...
    if xmax_clamped <= xmin_clamped or ymax_clamped <= ymin_clamped:
        print(f"Warning: Invalid crop coordinates for {label} detection: {xmin}, {ymin}, {xmax}, {ymax}")
        cropped_image_base64 = ""
    else:
...
# --- Classify Device Endpoint ---
@app.post("/classify-device/")
async def classify_device(file: UploadFile = File(...)):
    global last_detect_label

    if classification_model is None:
        raise HTTPException(status_code=500, detail="Classification model not loaded. Check server logs.")

    if last_detect_label is None:
        raise HTTPException(status_code=400, detail="No detection label found. Run /detect-device/ first.")
    ...
    if not results or not results[0].probs:
        raise HTTPException(status_code=500, detail="Classification prediction failed or no probabilities found.")
